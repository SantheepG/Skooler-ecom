...
const Reviewed = ({ reviewed, reload }) => {
  ...
  const deleteReview = async () => {
    try {
      const response = await DeleteReview(viewDelete);
      if (response.status === 200) {
        toast.success("Deleted");
        deleteHandler(viewDelete);
        reload();
      }
    } catch (error) {
      console.log(error);
      ...
      {reviewed !== null && reviewed.length !== 0 ? (
        <ul>
          {reviewed.map((review, index) => (
            ...
            >
              {viewDelete === review.id ? (
                <li
                  key={index}
                  value={review}
                  className="SlideDown bg-white text-left flex flex-col items-center text-gray-800"
                >
                  ...
                  <div class="mt-8 flex flex-col justify-center space-y-3 sm:flex-row sm:space-x-3 sm:space-y-0">
                    <button
                      class="whitespace-nowrap rounded-md bg-red-500 px-8 py-3 font-medium text-white"
